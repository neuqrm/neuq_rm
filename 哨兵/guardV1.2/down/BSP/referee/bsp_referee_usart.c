...
/* -------------- Enable Module Clock Source ----------------------------*/
	...
	RCC_APB1PeriphClockCmd(REFEREE_USART_CLK, ENABLE);
...
void USART3_IRQHandler(void)
{
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)
	{
		...
		USART_ITConfig(USART3, USART_IT_RXNE, DISABLE); //usart rx idle interrupt enabled
	}
	if(USART_GetITStatus(USART3, USART_IT_IDLE) != RESET)
	{
	 ...
	 //clear the idle pending flag
		...
		if(DMA_GetCurrentMemoryTarget(DMA1_Stream1) == 0)
		{
			...
			Realtime_RX_Length = REFEREE_BUFFER_LENGTH - DMA_GetCurrDataCounter(DMA1_Stream1);
			DMA1_Stream1->NDTR = (uint16_t)REFEREE_BUFFER_LENGTH; //relocate the dma memory pointer to the beginning position
