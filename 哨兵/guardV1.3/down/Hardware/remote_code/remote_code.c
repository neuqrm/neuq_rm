...
int lock_flag = 0;//判断是否该进行堵转判断，初始为零表示不应该判断
int count_1=1; //用于执行发射过程中的延时
//内部函数声明
		...
		switch (trigger_control_mode)                                               //遥控器右拨杆
				{
					...
					fric_angular=1500;
					count_1++;
					if(count_1>300)    												//300ms延时
					{
						trigger_speed = 100;
						count_1 = 1;  
						lock_flag = 1;	      //可以进行堵转判断
					}
					
					if((motor5.actual_speed<20&&motor5.actual_speed>-20)&&lock_flag)    						//堵转
					{ 
						static int count_=1;
					  count_++;
					...
					case 2: //最下面
					count_1=1;                       //保证拨回1时重新计数
					lock_flag = 0;								  //保证拨回1时不会误判防堵转
