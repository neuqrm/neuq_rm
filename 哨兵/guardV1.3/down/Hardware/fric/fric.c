...
void fric_PWM_configuration(void) //
{
    ...
    TIM_OCInitStructure.TIM_Pulse = 400;//修改
...
void auto_fire(void)
{
				...
				trigger_control(Kinematics.trigger.target_angular);
				//fric1_on(400);
				//fric2_on(400);
				fric1_on(1500);
				fric2_on(1500);
			//	fric1_on(Kinematics.fric.target_angular);
			//	fric2_on(Kinematics.fric.target_angular);
