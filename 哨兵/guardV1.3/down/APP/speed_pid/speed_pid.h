...
#define pid_init() \
				do{ \
					...
					pid_t.chassis_pid.speed_loop.ki = 0.2; \
					pid_t.chassis_pid.speed_loop.kd = 0; \
					                                     \
					/*底盘位置环*/                        \
					pid_t.chassis_pid.position_loop.kp = 0; \
					pid_t.chassis_pid.position_loop.ki = 0; \
					pid_t.chassis_pid.position_loop.kd = 0; \
                                               \
					/*拨弹轮速度环*/                      \
					pid_t.trigger_pid.speed_loop.kp = 4; \
					pid_t.trigger_pid.speed_loop.ki = 0.2; \
					pid_t.trigger_pid.speed_loop.kd = 0; \
                                               \
					/*云台yaw位置环*/                    \
					pid_t.yaw_pid.position_loop.kp = 0.3; \
					pid_t.yaw_pid.position_loop.ki = 0; \
					pid_t.yaw_pid.position_loop.kd = 2; \
					                                    \
					/*云台yaw速度环*/                    \
				  pid_t.yaw_pid.speed_loop.kp = 150; \
				  pid_t.yaw_pid.speed_loop.ki = 0; \
				  pid_t.yaw_pid.speed_loop.kd = 150; \
					                                 \
					/*云台pitch位置环*/                    \
					pid_t.pitch_pid.position_loop.kp = 0.3; \
					pid_t.pitch_pid.position_loop.ki = 0; \
					pid_t.pitch_pid.position_loop.kd = 0.2; \
					                                      \
					/*云台pitch速度环*/                    \
				  pid_t.pitch_pid.speed_loop.kp = 250; \
				  pid_t.pitch_pid.speed_loop.ki = 0.7; \
				  pid_t.pitch_pid.speed_loop.kd = 200; \
				}while(0)                            \
