...
volatile unsigned char store[500][10];   //
static int num = 0;//
...
void REFEREE_USART_IRQHandler(void)//使用的是串口中断，没有用dma中断，使用的模式是双缓冲但其实只用了一个缓冲区,buffer[0]
{
	...
	if(USART_GetITStatus(REFEREE_USART, USART_IT_IDLE) != RESET)
	{
	 ...
	 //clear the idle pending flag
		...
		else
		{
			...
			//观测接收信息，观测第3个字的序号是否连续，观测第5和第6位的命令码id是否对应
			if(num<500)
			{
				for(int j=0;j<10;j++)
				store[num][j] = Referee_Buffer[0][j];
				num++;
			}	
