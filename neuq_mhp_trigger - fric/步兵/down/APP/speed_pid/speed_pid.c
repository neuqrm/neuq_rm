	...
	switch(switch_flag)
	{
		...
		{		 
			if(vpid->pid_count>=average)
				vpid->pid_count=0;
			vpid->gimbal_err[vpid->pid_count] = 0.5*(vpid->err+vpid->last_err);
			vpid->pid_count++;
			for(int i=0;i<=(average-1);i++)
				vpid->average_err += (vpid->gimbal_err[i]);  
...
{
	switch_flag = FRIC;
	vpid_realize(&fric1.vpid,kp_fric1,ki_fric1,kd_fric1); 
	vpid_realize(&fric2.vpid,kp_fric2,ki_fric2,kd_fric2);
	switch_flag = NUL;
} //mhp222
