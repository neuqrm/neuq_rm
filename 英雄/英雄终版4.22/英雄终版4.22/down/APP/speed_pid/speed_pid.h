...
#define GIMBAL_Integral_max         300// 3000
#define GIMBAL_IntegralSeparation    100//20
#define GIMBAL_vPID_max              29500
#define AVERAGE                      10              //平均误差数组长度  


//摩擦轮 暂定
#define FIRE_Integral_max         3000            //抗积分饱和
#define FIRE_IntegralSeparation   500             //积分分离
#define FIRE_vPID_max             1900            //输出限幅
...
#define set_gimbal_speed(yaw_speed,pitch_speed) \
        do{                                     \
           gimbal_y.vpid.target_speed = yaw_speed*2; \
					 gimbal_y.target_speed = yaw_speed*2;       \
...
#define pid_init() \
				do{ \
					/*底盘速度环*/                        \
					pid_t.chassis_pid.speed_loop.kp = 4; \
					...
					pid_t.chassis_pid.position_loop.kd = 0; \
                                               \
					/*拨弹轮速度环*/                      \
					pid_t.trigger_pid.speed_loop.kp = 15; \
					pid_t.trigger_pid.speed_loop.ki = 1.7; \
					pid_t.trigger_pid.speed_loop.kd = 0; \
                                               \
					/*拨弹轮速度环*/                      \
					pid_t.trigger_pid.position_loop.kp = 0.25; \
					pid_t.trigger_pid.position_loop.ki = 0; \
					pid_t.trigger_pid.position_loop.kd = 0.15; \
                                               \
					/*云台yaw位置环*/                    \
					pid_t.yaw_pid.position_loop.kp = 150; \
					pid_t.yaw_pid.position_loop.ki = 0; \
					pid_t.yaw_pid.position_loop.kd = 0; \
					                                    \
					/*云台yaw速度环*/                    \
				  pid_t.yaw_pid.speed_loop.kp = 180; \
				  pid_t.yaw_pid.speed_loop.ki = 32; \
				  pid_t.yaw_pid.speed_loop.kd = 5; \
					                                 \
					/*云台pitch位置环*/                    \
					pid_t.pitch_pid.position_loop.kp = 0.04; \
					pid_t.pitch_pid.position_loop.ki = 0; \
					pid_t.pitch_pid.position_loop.kd = 2.5; \
					                                      \
					/*云台pitch速度环*/                    \
				  pid_t.pitch_pid.speed_loop.kp = 205; \
				  pid_t.pitch_pid.speed_loop.ki = 35; \
				  pid_t.pitch_pid.speed_loop.kd = 0; \
				  	                                      \
					/*摩擦轮速度环*/                    \
				  pid_t.fire_pid.speed_loop.kp  = 10; \
				  pid_t.fire_pid.speed_loop.ki = 0.6; \
				  pid_t.fire_pid.speed_loop.kd = 5; \
				}while(0)                            \
