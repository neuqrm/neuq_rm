...
void Encoder_Light_TIM4(u16 arr,u16 psc)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12| GPIO_Pin_13;          //GPIOA6和GPIOA7
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;                    //复用模式
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;              //速度100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;                //浮空	
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;                  //推挽复用输出
  GPIO_Init(GPIOD, &GPIO_InitStructure);                          //初始化PA6和PA7

    GPIO_PinAFConfig(GPIOD,GPIO_PinSource12,GPIO_AF_TIM4);           //GPIOA6复用为定时器3通道1
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource13,GPIO_AF_TIM4);           //GPIOA7复用为定时器3通道2
...
void Encoder_Light_TIM2(u16 arr,u16 psc)
{
  ...
  GPIO_PinAFConfig(GPIOA,GPIO_PinSource0,GPIO_AF_TIM2);           //GPIOA6复用为定时器3通道1
  GPIO_PinAFConfig(GPIOA,GPIO_PinSource1,GPIO_AF_TIM2);           //GPIOA7复用为定时器3通道2
...
{
	Encoder_Light_TIM2((4*ENCODER_PPR-4),0);
	Encoder_Light_TIM4((4*ENCODER_PPR-4),0);
}
...
u16 yu;
u32 Read_Encoder(int TIMx)//读取编码器得出脉冲
{
	...
	case 2:
	 ...
	 Encoder2_Timer_Overflow=0;   
                                                     //如果正转
	 Count = (u32)(Current_Count + (Enc_Timer_Overflow_one) * (4*ENCODER_PPR-4));       //计算出一个时间转过的脉冲数
	...
	TIM4->CNT=0;
	Encoder4_Timer_Overflow=0;                                                  //如果正转
	Count = (u32)(Current_Count + (Enc_Timer_Overflow_one) * (4*ENCODER_PPR-4));
     break; 
	}
	Count/=4;
	return Count;
