...
void send_gimbal_info_by_json(void)   
{
  json_t *root;
	...
	root = json_pack("[{sfsf}[ff]]",\
					  "yaw_angular", (Kinematics.yaw.actual_angle),\
						"pitch_angular", (Kinematics.pitch.actual_angle),\
						(Kinematics.yaw.actual_angle),\
						(Kinematics.pitch.actual_angle));   //(int),
	out = json_dumps(root, JSON_ENCODE_ANY);
	printf("%s\r\n", out);
	json_decref(root);
	//free(root);
...
{
//����   
Kinematics.fric1.actual_angular=7;
json_t *root;
	char *out;           //
	root = json_pack("[{sf}[f]]",\
						
					  "fric_angular", (Kinematics.fric1.actual_angular),\
						
						(Kinematics.fric1.actual_angular));   //(int),
		Kinematics.fric2.actual_angular=	Kinematics.fric1.actual_angular	;		
	out = json_dumps(root, JSON_ENCODE_ANY);
	printf("%s\r\n", out);
	json_decref(root);
	//free(root);
...
{ 
	...
	gimbal_obj = json_object_get( root, "gimbal" );
	item_obj = json_array_get( gimbal_obj, 0 );
	Kinematics.yaw.target_angle=0.01f*json_integer_value(item_obj);
	Kinematics.yaw.target_angle=Kinematics.yaw.target_angle/180*PI;
	item_obj = json_array_get( gimbal_obj, 1 );
	Kinematics.pitch.target_angle=0.01f*json_integer_value(item_obj);
	json_decref(item_obj);
	json_decref(gimbal_obj);
	json_decref(root);
}
...
void resolve_json_fric_command()
	...
	Kinematics.fric1.target_angular=1.0f*json_integer_value(item_obj);
	Kinematics.fric2.target_angular=Kinematics.fric1.target_angular;
	json_decref(item_obj);
