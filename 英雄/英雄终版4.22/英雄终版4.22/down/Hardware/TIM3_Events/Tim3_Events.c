...
void Robo_Move()
{
 ...
 /***** 各运动控制函数 ******/
	...
	if(TRIGGER_POS_EN)	
		...
		trigger_speed_angle(Trigger_Speed);                      //	拨弹轮速度控制
...
/***** 摩擦轮控制 *****/
		...
		if(FRIC_CLOSE_LOOP_EN==0)//开环
		{
		 fric1_on(FRIC1_Speed);                                          
		 fric2_on(FRIC2_Speed);
		}
	    else fric_control_close(FRIC1_Speed,FRIC2_Speed);		
