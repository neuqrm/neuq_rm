...
#include "kinematic.h"
#include "algorithm.h"
MOTOR_t motor1,motor2,motor3,motor4,motor5,motor6,gimbal_y,gimbal_p,fire1,fire2;
LOOPBACK loopback;
int angle_tf = 0;
...
/**
  * @breif 电机can回调赋值函数
	*/
void record_motor_callback_gimbal_y(MOTOR_t *motor, uint16_t angle, int16_t speed, int16_t current)
{
	if(motor->start_angle_flag==0)
	{
		motor->start_angle = angle;
		angle_tf = 4096 - motor->start_angle;
		motor->start_angle_flag++;	
	}

	motor->last_angle = motor->actual_angle;
	motor->actual_angle = angle +angle_tf;
...
void set_gimbal_current()
	...
	//电机目标电流为速度pid输出
	gimbal_y.target_current = gimbal_y.vpid.PID_OUT; //gimbal_y.vpid.PID_OUT;
