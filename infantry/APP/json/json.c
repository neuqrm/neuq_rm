...
void resolve_json_trigger_shoot_command()//mhp111
{ 
	int bullet_num=0;
	json_t *root;
	json_t *trigger_obj;
...
void resolve_json_steering_engine_command()
{ 
	int steering_engine_value=0;
	json_t *root;
	json_t *steering_engine_obj;
	json_t *item_obj;
	json_error_t error;
	root = json_loads(json_Buffer,0,&error);
	steering_engine_obj = json_object_get( root, "steering_engine" );
	item_obj = json_array_get( steering_engine_obj, 0 );
	steering_engine_value=1.0f*json_integer_value(item_obj); //10000;
	if(steering_engine_value==1)
		TIM_SetCompare3(TIM2, 200);
	if(steering_engine_value==0)
		TIM_SetCompare3(TIM2, 50);
	json_decref(item_obj);
	json_decref(steering_engine_obj);
	json_decref(root);
}//mhp222
/**
  *@brief 接受上位机调节pid参数
