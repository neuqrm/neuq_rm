...
{	
	if(Remote_control_mode>remote_min_value && Remote_control_mode<remote_max_value)		//判断遥控器是否开启
		control_mode = DJi_Remote_Control;								//标志位改为遥控模式
	else
		control_mode = auto_control;							//标志位改为自动模式
