...
/* -------------- Enable Module Clock Source ----------------------------*/
	...
	REFEREE_USART_PeriphClockCmd(REFEREE_USART_CLK, ENABLE);
...
void REFEREE_USART_IRQHandler(void)
{
	...
	if(USART_GetITStatus(REFEREE_USART, USART_IT_IDLE) != RESET)  //串口空闲中断标志
	{
		static uint16_t Realtime_RX_Length = 0;
	 //clear the idle pending flag 清除空闲挂起标志
		(void)REFEREE_USART->SR;
		(void)REFEREE_USART->DR;
		//Target is Memory0
		if(DMA_GetCurrentMemoryTarget(REFEREE_USART_DMA_STREAM) == 0)  //查询使用的缓存是0号缓存
		{
			...
			REFEREE_USART_DMA_STREAM->NDTR = (uint16_t)REFEREE_BUFFER_LENGTH; //relocate the dma memory pointer to the beginning position 将dma内存指针重新定位到起始位置
